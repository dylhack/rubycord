...
RSpec.describe RubyCord::Guild::AutoModRule do
  let(:data) do
    load_payload("automod.json")
  end
  ...
  it "returns actions" do
    ...
    expect(rule.actions.first).to be_a RubyCord::Guild::AutoModRule::Action
  end

  it "returns exempt roles" do
    expect(rule.exempt_roles).to be_a Array
    expect(rule.exempt_roles.first).to be_a RubyCord::Guild::Role
  end

  it "returns exempt channels" do
    expect(rule.exempt_channels).to be_a Array
    expect(rule.exempt_channels.first).to be_a RubyCord::Channel
  end
