    ...
    module GatewayHandler
      ...
      def connect_gateway(reconnect)
        Async do
          ...
          @mutex["gateway_#{shard_id}"].synchronize do
            ...
            @http = Client::HTTP.new(self)
            gateway_url =
              if reconnect
                @resume_gateway_url
              else
                _, gateway_response =
                  @http.request(RubyCord::Internal::Route.new("/gateway", "//gateway", :get)).wait
            ...
            begin
              ...
              begin
                self.connection =
                  Async::WebSocket::Client.connect(
                    endpoint,
                    headers: [["User-Agent", RubyCord::USER_AGENT]],
              ...
              when 4001, 4002, 4003, 4005, 4007
                raise ClientError.new(<<~ERROR), cause: e
                                                    ...
                                                    https://github.com/rubycord-lib/rubycord/issues
                                                  ERROR
              when 1001
      ...
      def handle_event(event_name, data)
        ...
        when "READY"
          ...
          @user = Client::User.new(self, data[:user])
        ...
        when "THREAD_MEMBER_UPDATE"
          ...
          if (member = thread.members[data[:id]])
            old =
              RubyCord::Guild::ThreadChannel::Member.new(
                self,
            ...
            old = nil
            member = RubyCord::Guild::ThreadChannel::Member.new(self, data, data[:guild_id])
        ...
        when "THREAD_MEMBERS_UPDATE"
          ...
          (data[:added_members] || []).each do |raw_member|
            member =
              RubyCord::Guild::ThreadChannel::Member.new(self, raw_member, data[:guild_id])
            thread.members[member.id] = member
            members << member
          end
          removed_members = (data[:removed_member_ids] || []).map do |id|
            thread.members.delete(id)
          end
          dispatch(:thread_members_update, thread, members, removed_members)
        when "STAGE_INSTANCE_CREATE"
          instance = RubyCord::Guild::StageChannel::StageInstance.new(self, data)
        ...
        when "STAGE_INSTANCE_UPDATE"
          ...
          old =
            RubyCord::Guild::StageChannel::StageInstance.new(
              self,
        ...
        when "INTEGRATION_DELETE"
          ...
          dispatch(:integration_delete, Client::Gateway::Event::IntegrationDeleteEvent.new(self, data))
        when "WEBHOOKS_UPDATE"
          dispatch(:webhooks_update, WebhooksUpdateEvent.new(self, data))
        when "INVITE_CREATE"
          dispatch(:invite_create, Guild::Invite.new(self, data, true))
        when "INVITE_DELETE"
          dispatch(:invite_delete, Client::Gateway::Event::InviteDeleteEvent.new(self, data))
        when "VOICE_STATE_UPDATE"
          ...
          if current.nil?
            old = nil
            current = Guild::VoiceChannel::VoiceState.new(self, data)
            guild.voice_states[data[:user_id]] = current
          else
            guild.voice_states.remove(data[:user_id]) if data[:channel_id].nil?
            old = Guild::VoiceChannel::VoiceState.new(self, current.instance_variable_get(:@data))
            current.send(:_set_data, data)
          end
        ...
        when "PRESENCE_UPDATE"
          ...
          guild.presences[data[:user][:id]] = User::Presence.new(self, data)
        when "MESSAGE_UPDATE"
          ...
          if data[:edited_timestamp].nil?
            ...
            dispatch(
              :message_pin_update,
              Client::Gateway::Event::MessagePinEvent.new(self, data, message)
            )
          else
            dispatch(
              :message_update,
              Client::Gateway::Event::MessageUpdateEvent.new(self, data, before, current)
            )
          end
        when "MESSAGE_DELETE"
          if (message = @messages[data[:id]])
        ...
        when "MESSAGE_DELETE_BULK"
          ...
          data[:ids].each do |id|
            if (message = @messages[id])
              ...
              messages.push(Client::Gateway::Event::UnknownDeleteBulkMessage.new(self, id, data))
            end
          end
        ...
        when "MESSAGE_REACTION_ADD"
          ...
          dispatch(:reaction_add, Client::Gateway::Event::ReactionEvent.new(self, data))
        when "MESSAGE_REACTION_REMOVE"
          ...
          dispatch(:reaction_remove, Client::Gateway::Event::ReactionEvent.new(self, data))
        when "MESSAGE_REACTION_REMOVE_ALL"
          if (target_message = @messages[data[:message_id]])
            target_message.reactions = []
          end
          dispatch(:reaction_remove_all, Client::Gateway::Event::ReactionRemoveAllEvent.new(self, data))
        when "MESSAGE_REACTION_REMOVE_EMOJI"
          ...
          dispatch(
            ...
            Client::Gateway::Event::ReactionRemoveEmojiEvent.new(self, data)
          )
        when "TYPING_START"
          dispatch(:typing_start, Client::Gateway::Event::TypingStartEvent.new(self, data))
        ...
        when "GUILD_SCHEDULED_EVENT_USER_ADD"
          ...
          dispatch(
            ...
            Client::Gateway::Event::ScheduledEventUserEvent.new(self, data)
          )
        ...
        when "AUTO_MODERATION_ACTION_EXECUTION"
          dispatch(
            ...
            Client::Gateway::Event::AutoModerationActionExecutionEvent.new(self, data)
          )
        when "AUTO_MODERATION_RULE_CREATE"
          dispatch(:auto_moderation_rule_create, Guild::AutoModRule.new(self, data))
        when "AUTO_MODERATION_RULE_UPDATE"
          dispatch(:auto_moderation_rule_update, Guild::AutoModRule.new(self, data))
        when "AUTO_MODERATION_RULE_DELETE"
          dispatch(:auto_moderation_rule_delete, Guild::AutoModRule.new(self, data))
        else
