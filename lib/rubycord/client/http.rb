  ...
  class Client
    class HTTP
      ...
      def initialize(client)
        @client = client
        @ratelimit_handler = Client::HTTP::RateLimitHandler.new(client)
      end
      ...
      # @async
      #
      # @param [RubyCord::Internal::Route] path The path to the resource.
      ...
      #
      # @raise [RubyCord::HTTPError] The request was failed.
      #
      ...
      # @param [Array<RubyCord::Attachment>] files The files to upload.
      # @param [String] audit_log_reason The audit log reason to send with the request.
      # @param [Hash] kwargs The keyword arguments.
      #
      # @return [Async::Task<Array(Net::HTTPResponse, Hash)>] The response and as JSON.
              ...
              request_io =
                StringIO.new(
                  cdn_http.get(
                    ...
                    { "Content-Type" => nil, "User-Agent" => RubyCord::USER_AGENT }
                  ).body
                )
              data << [
                "files[#{i}]",
...
require_relative "http/rate_limit_handler"
