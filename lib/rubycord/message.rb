...
module RubyCord
  ...
  class Message < RubyCord::DiscordModel
    ...
    attr_reader :id
    # @return [RubyCord::User, RubyCord::Guild::Member, Webhook::Message::Author] The user that sent the message.
    attr_reader :author
    ...
    # @return [Array<RubyCord::Attachment>] The attachments of the message.
    attr_reader :attachments
    # @return [Array<RubyCord::Embed>] The embeds of the message.
    attr_reader :embeds
    # @return [Array<RubyCord::Reaction>] The reactions of the message.
    ...
    attr_reader :type
    # @return [RubyCord::Message::Activity] The activity of the message.
    attr_reader :activity
    # @return [RubyCord::Application] The application of the message.
    attr_reader :application_id
    # @return [RubyCord::Message::Reference] The reference of the message.
    attr_reader :message_reference
    # @return [RubyCord::Message::Flag] The flag of the message.
    # @see RubyCord::Message::Flag
    attr_reader :flag
    # @return [RubyCord::Message::Sticker] The sticker of the message.
    attr_reader :stickers
    # @return [RubyCord::Message::Interaction] The interaction of the message.
    attr_reader :interaction
    # @return [RubyCord::Guild::ThreadChannel] The thread channel of the message.
    attr_reader :thread
    # @return [Array<Array<RubyCord::Component>>] The components of the message.
    ...
    # @!attribute [r] embed
    #   @return [RubyCord::Embed] The embed of the message.
    ...
    #
    # @return [RubyCord::Message::Reference] The reference object.
    #
    ...
    # @async
    #
    # @param [RubyCord::Emoji] emoji The emoji to react with.
    #
    # @return [Async::Task<void>] The task.
    #
    ...
    # @async
    #
    # @param [RubyCord::Emoji] emoji The emoji to remove.
    #
    # @return [Async::Task<void>] The task.
    #
    ...
    #
    # @param [RubyCord::Emoji] emoji The emoji to remove.
    # @param [RubyCord::Guild::Member] member The member to remove the reaction from.
    #
    # @return [Async::Task<void>] The task.
    #
    ...
    # @async
    #
    # @param [RubyCord::Emoji, RubyCord::PartialEmoji] emoji The emoji to fetch.
    # @param [Integer, nil] limit The maximum number of users to fetch. `nil` for no limit.
    # @param [RubyCord::Snowflake, nil] after The ID of the user to start fetching from.
    #
    # @return [Async::Task<Array<RubyCord::User>>] The users.
    #
    def fetch_reacted_users(
      emoji,
      limit: nil,
      after: RubyCord::Snowflake.new("0")
    )
      Async do
    ...
    # @async
    #
    # @param (see RubyCord::Channel#start_thread)
    #
    # @return [Async::Task<RubyCord::Guild::ThreadChannel>] <description>
    #
    ...
    def _set_data(data)
      ...
      if data[:guild_id]
        ...
        @dm = RubyCord::DMChannel.new(@client, data[:channel_id])
        @guild_id = nil
      end

      if data[:member].nil? && data[:webhook_id]
        @webhook_id = Snowflake.new(data[:webhook_id])
        @author = Guild::Webhook::Message::Author.new(data[:author])
        ...
        @author =
          guild&.members&.get(data[:author][:id]) ||
            RubyCord::Guild::Member.new(@client, @guild_id, data[:author], data[:member])
      end
