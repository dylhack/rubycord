...
module RubyCord
  ...
  class Channel < RubyCord::DiscordModel
    ...
    #
    # @param [RubyCord::Channel] other The channel to check.
    #
    ...
    def self.make_channel(client, data, no_cache: false)
      descendants.each do |klass|
        if !klass.channel_type.nil? && klass.channel_type == data[:type] && klass != RubyCord::Guild::ForumChannel::Post
      ...
      RubyCord::Guild::Channel.new(client, data)
    end
