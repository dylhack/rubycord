...
require_relative "client/gateway_handler"
require_relative "client/command_handler"
require_relative "client/http"
require_relative "client/gateway"
require_relative "client/extension"
...
module RubyCord
  ...
  #
  class Client
    include Client::GatewayHandler
    include Client::CommandHandler
    # @return [RubyCord::Client::Gateway::Intents] The intents that the client is currently using.
    attr_accessor :intents
    # @return [RubyCord::Application] The application that the client is using.
    # @return [nil] If never fetched application by {#fetch_application}.
    attr_reader :application
    # @return [RubyCord::Client::HTTP] The http client.
    attr_reader :http
    ...
    attr_reader :token
    # @return [RubyCord::AllowedMentions] The allowed mentions that the client is using.
    attr_reader :allowed_mentions
    # @return [RubyCord::Client::User] The client user.
    attr_reader :user
    # @return [RubyCord::Internal::Dictionary{RubyCord::Snowflake => RubyCord::Guild}] A dictionary of guilds.
    attr_reader :guilds
    # @return [RubyCord::Internal::Dictionary{RubyCord::Snowflake => RubyCord::User}] A dictionary of users.
    attr_reader :users
    # @return [RubyCord::Internal::Dictionary{RubyCord::Snowflake => RubyCord::Channel}] A dictionary of channels.
    attr_reader :channels
    # @return [RubyCord::Internal::Dictionary{RubyCord::Snowflake => RubyCord::Emoji}] A dictionary of emojis.
    attr_reader :emojis
    # @return [RubyCord::Internal::Dictionary{RubyCord::Snowflake => RubyCord::Message}] A dictionary of messages.
    attr_reader :messages
    # @return [Array<RubyCord::Command>] The commands that the client is using.
    ...
    attr_reader :status
    # @return [Hash{String => RubyCord::Extension}] The loaded extensions.
    attr_reader :extensions
    # @return [Hash{Integer => RubyCord::Client::Gateway::Shard}] The shards of the client.
    attr_reader :shards
    # @private
    # @return [Hash{RubyCord::Snowflake => RubyCord::Command}] The commands on the top level.
    ...
    #   @return [nil] If not connected to the gateway.
    # @!attribute [r] shard
    #   @return [RubyCord::Client::Gateway::Shard] The current shard. This is implemented with Thread variables.
    ...
    #
    # @param [RubyCord::AllowedMentions] allowed_mentions The allowed mentions that the client is using.
    # @param [RubyCord::Client::Gateway::Intents] intents The intents that the client is currently using.
    ...
    def initialize(
      ...
      @intents = (intents or Client::Gateway::Intents.default)
      @events = {}
      @api_version = nil
      @logger =
        logger || Logger.new($stdout, progname: "discorb", level: Logger::ERROR)
      @user = nil
      @users = RubyCord::Internal::Dictionary.new
      @channels = RubyCord::Internal::Dictionary.new
      @guilds = RubyCord::Internal::Dictionary.new(sort: ->(k) { k[0].to_i })
      @emojis = RubyCord::Internal::Dictionary.new
      @messages = RubyCord::Internal::Dictionary.new(limit: message_caches)
      @application = nil
      @last_s = nil
    ...
    #
    # @return [RubyCord::EventHandler] The event.
    #
    ...
    def dispatch(event_name, *args)
      Async do
        ...
        if respond_to?("on_#{event_name}")
          ...
          # Add the id method to the singleton class of event_method
          def event_method.id
            "method"
          end
        ...
        events.each do |block|
          Async do
            Async(annotation: "Discorb event: #{event_name}") do |_task|
              if block.is_a?(RubyCord::EventHandler) && block.metadata[:once]
    ...
    #
    # @return [Async::Task<RubyCord::User>] The user.
    #
    # @raise [RubyCord::NotFoundError] If the user doesn't exist.
    #
    def fetch_user(id)
      Async do
        _resp, data =
          @http.request(
            RubyCord::Internal::Route.new("/users/#{id}", "//users/:user_id", :get)
          ).wait
        User.new(self, data)
      end
    end
    ...
    #
    # @return [Async::Task<RubyCord::Channel>] The channel.
    #
    # @raise [RubyCord::NotFoundError] If the channel doesn't exist.
    #
    def fetch_channel(id)
      Async do
        _resp, data =
          @http.request(
            RubyCord::Internal::Route.new("/channels/#{id}", "//channels/:channel_id", :get)
          ).wait
    ...
    # @param [#to_s] id <description>
    #
    # @return [Async::Task<RubyCord::Guild>] The guild.
    #
    # @raise [RubyCord::NotFoundError] If the guild doesn't exist.
    #
    def fetch_guild(id)
      Async do
        _resp, data =
          @http.request(
            RubyCord::Internal::Route.new("/guilds/#{id}", "//guilds/:guild_id", :get)
          ).wait
    ...
    #
    # @return [Async::Task<RubyCord::Guild::Invite>] The invite.
    #
    def fetch_invite(code, with_count: true, with_expiration: true)
      Async do
        ...
        Guild::Invite.new(self, data, false)
      end
    end
    ...
    #
    # @return [Async::Task<RubyCord::Application>] The application.
    #
    ...
    # @async
    #
    # @return [Async::Task<Array<RubyCord::Sticker::Pack>>] The packs.
    #
    def fetch_nitro_sticker_packs
      Async do
        _resp, data =
          @http.request(
            RubyCord::Internal::Route.new("/sticker-packs", "//sticker-packs", :get)
          ).wait
    ...
    #
    # @param [RubyCord::Activity] activity The activity to update.
    ...
    #
    # @raise [RubyCord::TimeoutError] If the event didn't occur in time.
    #
    ...
    # Load the extension.
    #
    # @param [Class, RubyCord::Client::Extension] ext The extension to load.
    # @param [Array<Object>] args The arguments to pass to the `ext#initialize`.
    #
    def load_extension(ext, *args)
      case ext
      when Class
        unless ext < RubyCord::Client::Extension
          raise ArgumentError, "#{ext} is not a extension"
        end

        ins = ext.new(self, args)
      when RubyCord::Client::Extension
        ins = ext
      else
        raise ArgumentError, "#{ext} is not a extension"
      end
      ...
      cls = ins.class
      cls.loaded(self, *args) if cls.respond_to? :loaded
    ...
    def before_run(token)
      require "json"
      options =
        JSON.parse(T.must(ENV.fetch("DISCORB_CLI_OPTIONS", nil)), symbolize_names: true)
    ...
    def run_setup(token)
      ...
      if ENV.fetch("DISCORB_SETUP_SCRIPT", nil) == "true"
        ...
        send(:on_setup) if respond_to? :on_setup
      end
    end
      ...
      if shards.nil?
        ...
        @shards =
          shards.each_with_index.to_h do |shard, i|
            [shard, RubyCord::Client::Gateway::Shard.new(self, shard, shard_count, i)]
          end
